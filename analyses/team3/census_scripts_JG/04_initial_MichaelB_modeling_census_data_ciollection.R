...
age_vars <- get_acs(geography = "zcta", variables = c(males_55_59 = "B01001_017",
...
pop_vars <- get_acs(geography = "zcta", variables = c(tot_pop = "B01001_001",
                                                      hispanic = "B03003_003",
                                                      nh_white = "B03002_003",
                                                      nh_black = "B03002_004",
                                                      nh_aian = "B03002_005",
...
econ_vars <- get_acs(geography = "zcta", variables = c(mhi = "B19013_001"), state = "PA",
                     year = 2019)

disability_vars <- get_acs(geography = "zcta", table = "B18101", state = "PA", year = 2019)
...
#bring zip and county shapes

zip_shp <- zctas()
cty_shp <- counties(state = "PA")

cty_shp <- cty_shp %>% 
        filter(NAME %in% c("Philadelphia", "Chester", "Delaware", "Montgomery"))

mont_zips <- st_intersection(zip_shp, cty_shp)
mont_zips <- mont_zips %>% 
        select(1:2)

montgomery_vars <- big_table %>% 
        inner_join(mont_zips, by = c("GEOID" = "GEOID20")) %>% 
        st_as_sf()


saveRDS(montgomery_vars, "analyses/team3/team3_data/census_model_data/philly_del_chester_subset_zip_vars.RDS")
st_write(montgomery_vars, "analyses/team3/team3_data/census_model_data/philly_del_chester_subset_zip_vars.gpkg", 
         delete_dsn = TRUE)

write_csv(montgomery_vars %>% 
                  as_tibble() %>% 
                  select(-geometry), "analyses/team3/team3_data/census_model_data/philly_del_chester_subset_zip_vars.csv")